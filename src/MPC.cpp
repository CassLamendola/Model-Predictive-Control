...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Calculate cost of change to make turns and acceleration smoother
    for (int t=0; t<N-1; t++) {
      fg[0] += delta_w * CppAD::pow(vars[delta_start+t], 2);
      fg[0] += a_w * CppAD::pow(vars[a_start+t], 2);
    }
    // Calculate cost of change based on last change to make sequential actuations smoother
    for (int t=0; t<N-2; t++) {
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // state size * number of timesteps + actuators size * number of timesteps-1
  size_t n_vars = state.size() * N + 2 * (N - 1);

  // Number of constraints
  // state size * number of timesteps;
